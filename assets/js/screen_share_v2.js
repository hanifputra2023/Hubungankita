...
class NativeScreenShareSession {
    ...
    async startSharing(useGamingMode = false) {
        ...
        console.log('[ScreenShare] Mulai berbagi layar. isApk:', isApk, 
                    '| window.Capacitor:', !!window.Capacitor, 
                    '| Plugins:', window.Capacitor?.Plugins ? Object.keys(window.Capacitor.Plugins) : 'none',
                    '| UA:', navigator.userAgent);

        if (!isApk) {
            console.log('[ScreenShare] Berjalan di browser/bukan native, menggunakan fallback browser sharing...');
        ...
        try {
            ...
            const captureOpts = useGamingMode
                ? { mode: 'webrtc', fps: 30, width: 960, height: 540, videoBitrate: 1200000, audioBitrate: 128000 }
                : { mode: 'webrtc', fps: 24, width: 854, height: 480, videoBitrate: 600000, audioBitrate: 96000 };
    ...
    async _startBrowserSharing(useGamingMode = false) {
        ...
        try {
            ...
            let finalOffer = this.pc.localDescription.sdp;
            finalOffer = this._optimizeSdpCodecs(finalOffer);
    ...
    _onIncomingShareScreen(data) {
        if (this.status !== 'idle') {
            if (data.session_id && this.sessionId && String(data.session_id) !== String(this.sessionId)) {
                console.log('[ScreenShare] Ada sesi baru dari pasangan, memulihkan state dari sesi lama...');
                this.stopSession();
            } else {
        ...
        this._pendingCandidates = []; // Reset antrean candidates untuk sesi baru ini
    ...
    async _startViewerConnection(sdpOffer, pendingCandidates = []) {
        if (this.status === 'connecting' || this.status === 'viewing') {
            console.warn('[ScreenShare Viewer] Koneksi sudah berjalan/terhubung. Abaikan pemanggilan ganda.');
            return;
        }
        this.role = 'viewer';
        this.status = 'connecting';
        ...
        try {
            ...
            let normOffer = this._normalizeSdp(sdpOffer);
            normOffer = this._optimizeSdpCodecs(normOffer);
            await this.pc.setRemoteDescription({ type: 'offer', sdp: normOffer });
            this._remoteDescSet = true;

            // Terapkan ICE candidates yang pending (gabungan dari payload server dan antrean Pusher yang masuk saat modal terbuka)
            const allCandidates = [...pendingCandidates, ...this._pendingCandidates];
            const seenCandidates = new Set();

            for (const c of allCandidates) {
                if (!c) continue;
                const candKey = typeof c === 'object' ? (c.candidate || JSON.stringify(c)) : String(c);
                if (seenCandidates.has(candKey)) continue;
            ...
            let finalAnswer = this.pc.localDescription.sdp;
            finalAnswer = this._optimizeSdpCodecs(finalAnswer);
    ...
    async _handleIncomingSignal(data) {
        ...
        if (signal.type === 'sdp-answer' && this.role === 'sharer') {
            ...
            if (this.pc) {
                try {
                    let normAnswer = this._normalizeSdp(signal.sdp);
                    normAnswer = this._optimizeSdpCodecs(normAnswer);
        ...
        if (signal.type === 'ice-candidate') {
            ...
            // Perbaikan: jika kita penerima (viewer atau modal undangan terbuka), simpan ke pending candidates jika PC belum siap
            if ((this.role === 'viewer' || (!this.role && this._incomingModalOpen)) && senderRole === 'sharer') {
                if (this.pc) {
                    ...
                    // PC belum dibuat (User belum klik "Tonton") -> simpan kandidat agar tidak terbuang!
                    this._pendingCandidates.push(candidate);
                    console.log('[ScreenShare Viewer] ICE candidate disimpan ke antrean pending (PC belum siap):', candidate.sdpMid || 'cand');
                }
    ...
    _onRemoteSessionEnded() {
        ...
        // Bersihkan native listener & pending candidates jika ada
        if (this.nativeSignalListener) {
            try { this.nativeSignalListener.remove(); } catch (e) { }
            this.nativeSignalListener = null;
        }
        ...
        // Hentikan plugin native jika kita adalah sharer
        if (this.role === 'sharer' && window.Capacitor?.Plugins?.ScreenShare) {
            try { window.Capacitor.Plugins.ScreenShare.stopNativeWebRTC(); } catch (e) { }
        }
        ...
        // Hentikan local audio/video stream (browser sharer)
        if (this.localStream) {
            try {
                this.localStream.getTracks().forEach(t => t.stop());
            } catch (e) { }
    ...
    toggleZoom() {
        this.zoomMode = (this.zoomMode === 'fit') ? 'fill' : 'fit';
        const videoEl = document.getElementById('ss-remote-video');
        if (videoEl) {
            this._adjustVideoLayout(videoEl);
        }
        
        // Update warna button zoom
        const zoomBtn = document.getElementById('ss-btn-zoom');
        if (zoomBtn) {
            if (this.zoomMode === 'fill') {
                zoomBtn.style.color = '#f43f5e'; // Highlight rose
            } else {
                zoomBtn.style.color = '';
            }
        }
        this._showToast(this.zoomMode === 'fill' ? 'Mode Zoom: Penuh Layar' : 'Mode Zoom: Fit Layar');
    }

    toggleMic() {
        this.isMicMuted = !this.isMicMuted;
        
        // Mute browser stream audio tracks jika sharer via browser
        if (this.localStream) {
            this.localStream.getAudioTracks().forEach(t => { t.enabled = !this.isMicMuted; });
        }
        
        // Mute Android native plugin audio track jika sharer via APK
        if (window.Capacitor?.Plugins?.ScreenShare?.toggleMute) {
            window.Capacitor.Plugins.ScreenShare.toggleMute().catch(() => {});
        }

        this._updateMicMuteUI(this.isMicMuted);
        this._showToast(this.isMicMuted ? 'Mikrofon dibisukan 🎤❌' : 'Mikrofon aktif 🎤');
    ...
    toggleRemoteAudio() {
        ...
        this.isRemoteMuted = videoEl.muted;

        this._updateRemoteMuteUI(this.isRemoteMuted);
        this._showToast(this.isRemoteMuted ? 'Suara pasangan dibisukan 🔇' : 'Suara pasangan aktif 🔊');
        return this.isRemoteMuted;
    }

    _updateMicMuteUI(isMuted) {
        const iconOn = document.getElementById('ss-icon-mic-on');
        const iconOff = document.getElementById('ss-icon-mic-off');
        const textEl = document.getElementById('ss-text-mute-mic');
        const btn = document.getElementById('ss-btn-mute-mic');

        if (iconOn) iconOn.classList.toggle('hidden', isMuted);
        if (iconOff) iconOff.classList.toggle('hidden', !isMuted);
        if (textEl) textEl.textContent = isMuted ? 'Buka Bisu Mikrofon' : 'Bisu Mikrofon Saya';
        if (btn) {
            btn.classList.toggle('bg-rose-500/20', isMuted);
            btn.classList.toggle('border-rose-500/40', isMuted);
        }
    }

    _updateRemoteMuteUI(isMuted) {
        const iconOn = document.getElementById('ss-icon-audio-on');
        const iconOff = document.getElementById('ss-icon-audio-off');
    ...
    _adjustVideoLayout(videoEl) {
        ...
        const isMobile = window.innerWidth < 768 || this._isCapacitorNative();

        if (isLandscape) {
            // Jika konten lanskap (game mendatar), wadah harus 16/9 agar muat penuh
            container.style.aspectRatio = '16/9';
            videoEl.style.objectFit = (this.zoomMode === 'fill') ? 'cover' : 'contain';
        } else {
            // Jika konten potret
            if (isMobile) {
    ...
    _updateUI(state) {
        ...
        const btnMuteMic = document.getElementById('ss-btn-mute-mic');
        if (btnMuteMic) {
            btnMuteMic.classList.toggle('hidden', state !== 'sharing');
        }
