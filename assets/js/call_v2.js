...
class RelationshipCall {
    constructor() {
        ...
        // Tracking versi SDP screen share untuk polling fallback
        // Mencegah pemrosesan screen-offer/screen-answer yang sama berulang kali
        this._lastPolledScreenOffer = null;
        this._lastPolledScreenAnswer = null;
    ...
    async pollCallStatus() {
        try {
            ...
            // ── FALLBACK SIGNALING: Screen Share via DB ──
            // Karena pollCallStatus sudah berjalan secara berkala, memproses data ini
            // TIDAK menambah request HTTP baru (0% tambahan hits). Sangat krusial jika Pusher di APK native tidak aktif.
            await this._pollScreenShareSignals(data);
    ...
    /**
     * Polling fallback untuk screen share signaling via DB.
     * Dipanggil di setiap iterasi pollCallStatus saat panggilan aktif.
     * Memproses screen_offer dan screen_answer yang disimpan di DB
     * sebagai fallback Pusher (kritis untuk APK native).
     * @param {object} data - Data hasil polling dari server
     */
    async _pollScreenShareSignals(data) {
        if (this.status !== 'active') return;

        // ── Fallback screen-offer: penerima mendapat offer dari DB ──
        // Kondisi: ada screen_offer di DB, kita bukan yang melakukan screen share,
        // dan offer ini belum pernah kita proses sebelumnya
        if (data.screen_offer && !this.isScreenSharing && data.screen_offer !== this._lastPolledScreenOffer) {
            console.log('[ScreenShare Poll] screen-offer baru ditemukan di DB — memproses sebagai penerima...');
            this._lastPolledScreenOffer = data.screen_offer;
            this._lastPolledScreenAnswer = null; // reset answer tracker untuk sesi baru
            await this.handleRemoteScreenOffer(data.screen_offer);
            return; // cukup proses offer dulu
        }

        // ── Fallback screen-answer: host mendapat answer dari DB ──
        // Kondisi: ada screen_answer di DB, kita yang melakukan screen share (isScreenSharing),
        // kita di APK native, dan answer ini belum pernah kita proses
        if (data.screen_answer && this.isScreenSharing && data.screen_answer !== this._lastPolledScreenAnswer) {
            console.log('[ScreenShare Poll] screen-answer baru ditemukan di DB — host memproses answer...');
            this._lastPolledScreenAnswer = data.screen_answer;
            await this.handleRemoteScreenAnswer(data.screen_answer);
        }

        // ── Deteksi screen share berhenti via DB: screen_offer null tapi sebelumnya ada ──
        if (!data.screen_offer && this._lastPolledScreenOffer !== null && this._remoteScreenSharing) {
            console.log('[ScreenShare Poll] screen_offer hilang dari DB — screen share berhenti, restore layout...');
            this._lastPolledScreenOffer = null;
            this._lastPolledScreenAnswer = null;
    ...
    async handleRemoteScreenOffer(sdp) {
        ...
        try {
            ...
            this.screenPeerConnection.onicecandidate = (event) => {
                if (event.candidate) {
                    // PERBAIKAN BUG: Serialisasi RTCIceCandidate ke plain object sebelum dikirim.
                    // Native plugin addIceCandidate() mengambil field 'candidate', 'sdpMid', 'sdpMLineIndex'
                    // secara eksplisit. Jika dikirim RTCIceCandidate object mentah, field-field tersebut
                    // tidak terbaca oleh JSON.stringify di sisi server (karena non-enumerable), sehingga
                    // native plugin menerima object kosong dan ICE P2P gagal terbentuk.
                    ...
                    console.log('[WebRTC Screen] Mengirim local candidate (plain) ke host:', plainCandidate);
                    this.sendScreenSignal('screen-candidate', plainCandidate);
                }
            };
            ...
            // Wait for ICE gathering to complete (Vanilla ICE) to ensure all candidates are embedded in SDP
            console.log('[WebRTC Screen] Menunggu ICE gathering selesai...');
            if (this.screenPeerConnection.iceGatheringState !== 'complete') {
                await new Promise((resolve) => {
                    const checkState = () => {
                        if (this.screenPeerConnection.iceGatheringState === 'complete') {
                            this.screenPeerConnection.removeEventListener('icegatheringstatechange', checkState);
                            resolve();
                        }
                    };
                    this.screenPeerConnection.addEventListener('icegatheringstatechange', checkState);
                    // Safe timeout 3 detik
                    setTimeout(() => {
                        this.screenPeerConnection.removeEventListener('icegatheringstatechange', checkState);
                        resolve();
                    }, 3000);
            ...
            await this.sendScreenSignal('screen-answer', this.screenPeerConnection.localDescription.sdp);
        } catch (e) {
    ...
    async handleRemoteScreenAnswer(sdp) {
        ...
        if (!window.Capacitor || !window.Capacitor.Plugins || !window.Capacitor.Plugins.ScreenShare) return;

        // PERBAIKAN BUG: APK menerima screen-answer tapi native peerConnection bisa saja
        // belum siap (masih dalam proses ICE gathering atau bahkan sudah di-release).
        // Tambahkan retry dengan exponential backoff (max 5x, interval 1s → 2s → 3s → 4s → 5s).
        // Ini memastikan setRemoteAnswer berhasil meski answer tiba sangat cepat.
        const MAX_RETRIES = 5;
        let attempt = 0;
        while (attempt < MAX_RETRIES) {
            try {
                await window.Capacitor.Plugins.ScreenShare.setRemoteAnswer({ sdp: sdp });
                console.log(`[WebRTC Screen] Remote answer berhasil diset ke native plugin (percobaan ke-${attempt + 1}).`);
                this._screenAnswerRetryCount = 0; // reset counter setelah sukses
                return;
            } catch (e) {
                attempt++;
                const delayMs = attempt * 1000; // 1s, 2s, 3s, 4s, 5s
                console.warn(`[WebRTC Screen] setRemoteAnswer gagal (percobaan ${attempt}/${MAX_RETRIES}), retry dalam ${delayMs}ms:`, e.message || e);
                if (attempt < MAX_RETRIES) {
                    await new Promise(r => setTimeout(r, delayMs));
                } else {
                    console.error('[WebRTC Screen] setRemoteAnswer GAGAL setelah semua percobaan. Black screen mungkin terjadi.');
                }
            }
        }
    }
    ...
    async handleRemoteScreenCandidate(candidate) {
        ...
        try {
            ...
            // PERBAIKAN BUG: Sebelumnya logika ini salah mengecek this.isScreenSharing pada sisi PENERIMA.
            // this.isScreenSharing = true hanya di PENGIRIM/HOST screen share.
            // PENERIMA tidak screen sharing, jadi kondisi ini selalu false di penerima.
            // Solusi: Jika kita adalah HOST screen share native (isScreenSharing && isApk),
            // route candidate ke native plugin. Jika kita adalah PENERIMA, route ke screenPeerConnection.
            if (this.isScreenSharing && isApk && window.Capacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.ScreenShare) {
                // Kita adalah HOST screen share native — candidate ini dari penerima, kirim ke native plugin
                await window.Capacitor.Plugins.ScreenShare.addIceCandidate({ candidate: candidate });
                console.log('[WebRTC Screen] Candidate (dari penerima) ditambahkan ke native plugin HOST.');
            } else {
                // Kita adalah PENERIMA screen share — candidate ini dari host, tambah ke screenPeerConnection
                if (!this.screenPeerConnection || this.screenPeerConnection.remoteDescription === null) {
                    ...
                    const signature = JSON.stringify(candidate);
                    if (!this.screenProcessedCandidates) this.screenProcessedCandidates = new Set();
                    if (!this.screenProcessedCandidates.has(signature)) {
                        this.screenProcessedCandidates.add(signature);
                        await this.screenPeerConnection.addIceCandidate(new RTCIceCandidate(candidate));
                        console.log('[WebRTC Screen] Candidate ditambahkan ke screenPeerConnection PENERIMA.');
                    }
                }
            }
        } catch (e) {
    ...
    terminateCallLocally(message = "") {
        ...
        this.isScreenSharing = false;
        // Reset tracker polling screen share agar sesi berikutnya bersih
        this._lastPolledScreenOffer = null;
        this._lastPolledScreenAnswer = null;
