...
class RelationshipCall {
        ...
        if (isApk) {
            if (window.Capacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.ScreenShare) {
                try {
                    ...
                    const captureOpts = useGamingMode ? { scale: 0.65, fps: 30, quality: 75 } : { scale: 0.45, fps: 15, quality: 55 };
                        ...
                        const drawFrame = () => {
                            ...
                            if (mjpegImg.naturalWidth) {
                                if (canvas.width !== mjpegImg.naturalWidth || canvas.height !== mjpegImg.naturalHeight) {
                                    ...
                                    this.screenStream = canvas.captureStream(useGamingMode ? 30 : 15);
                                                    ...
                                                    .then(() => {
                                                        ...
                                                        try {
                                                            ...
                                                            if (parameters.encodings.length > 0) {
                                                                parameters.encodings[0].maxBitrate = useGamingMode ? 1800000 : 400000;
                                                                parameters.encodings[0].maxFramerate = useGamingMode ? 30 : 15;
                            ...
                            // Sesuaikan interval drawFrame ke ~15 FPS (~66ms) atau ~30 FPS (~33ms)
                            setTimeout(drawFrame, useGamingMode ? 33 : 66);
                        };
                        ...
                        if (this.peerConnection) {
                            ...
                            if (videoSender) {
                                ...
                                try {
                                    ...
                                    if (parameters.encodings.length > 0) {
                                        parameters.encodings[0].maxBitrate = useGamingMode ? 1800000 : 400000; // Limit ke 1.8 Mbps atau 400 kbps
                                        parameters.encodings[0].maxFramerate = useGamingMode ? 30 : 15;
    ...
    toggleVideoLayoutMode() {
        ...
        this._videoLayoutMode = (this._videoLayoutMode + 1) % 3;
        ...
        if (btnLayout) {
            const labels = ["Fit", "Zoom", "Rotasi"];
            const textSpan = btnLayout.querySelector('.mode-label');
            if (textSpan) {
                textSpan.textContent = labels[this._videoLayoutMode];
            }
        }
    ...
    applyVideoLayoutMode() {
        ...
        [remoteVideo, overlayVideo].forEach(v => {
            ...
            if (mode === 0) {
                ...
                v.style.width = '100dvh';
                v.style.height = '100vw';
                v.style.objectFit = 'contain';
                v.classList.add('object-contain');
