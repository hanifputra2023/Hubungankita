...
class RelationshipCall {
    ...
    handleRemoteScreenShareToggle(active) {
        ...
        if (active) {
            ...
            // ── 7. Deteksi orientasi stream → sesuaikan object-fit fallback ──
            this._adaptRemoteVideoLayout();

            // Pasang listener resize agar auto-adapt saat orientasi berubah di tengah sesi
            if (remoteVideo && !remoteVideo._ssResizeHandler) {
                remoteVideo._ssResizeHandler = () => {
                    if (this._remoteScreenSharing) this._adaptRemoteVideoLayout();
                };
                remoteVideo.addEventListener('resize', remoteVideo._ssResizeHandler);
            }
            ...
            // ── Cabut listener resize ──
            if (remoteVideo && remoteVideo._ssResizeHandler) {
                remoteVideo.removeEventListener('resize', remoteVideo._ssResizeHandler);
                remoteVideo._ssResizeHandler = null;
            }
     ...
     * Sesuaikan overlay widget aspect ratio agar tidak ada ruang kosong.
     * Retry otomatis jika dimensi belum tersedia.
     */
    _adaptRemoteVideoLayout() {
        ...
        console.log(`[ScreenShare] Stream: ${isLandscapeStream ? 'Landscape' : 'Portrait'} (${vw}x${vh}), Viewport: ${viewportWidth}x${viewportHeight}`);
