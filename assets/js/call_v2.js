...
class RelationshipCall {
    ...
    handleRemoteScreenShareToggle(active) {
        ...
        if (active) {
            ...
            // ── Expand card: ubah ke portrait-mode agar konten HP muat maksimal ──
            // Karena layar HP portrait (9:16), card perlu tinggi dan tidak terlalu lebar.
            if (expandCard) {
                // Simpan class asli agar bisa di-restore
                expandCard.dataset.origClass = expandCard.className;

                // Hapus pembatas lebar lama, tambah gaya portrait screen share
                expandCard.classList.remove('max-w-sm');
                expandCard.classList.add('max-w-sm');           // tetap narrow = portrait feel
                ...
                expandCard.style.padding      = '0';            // video butuh ruang penuh
            }

            // ── Overlay widget: ubah aspect ratio agar sesuai portrait ──
            if (overlayContainer) {
