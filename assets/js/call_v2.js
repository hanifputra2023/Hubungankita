...
class RelationshipCall {
    ...
    init() {
        ...
        this._onlineHandler = () => {
            console.log('[Call] Internet terhubung kembali. Melanjutkan polling...');
        ...
        this._offlineHandler = () => {
            console.log('[Call] Internet terputus. Menangguhkan polling...');
            this.stopPolling();
        };
    ...
    startActivePolling() {
        this.stopPolling();
        // Polling ringan setiap 4 detik selama panggilan aktif:
    ...
    startReconnectingPolling() {
        this.stopPolling();
        // Polling lebih sering saat dialing, ringing, reconnecting (setiap 2 detik)
        this.pollInterval = setInterval(() => {
            this.pollCallStatus();
        }, 2000);
    }
    ...
    destroy() {
        console.log('[Call] Menghancurkan instance RelationshipCall (hard reset)...');
        ...
        // 3. Matikan local media stream tracks
        if (this.localStream) {
            try {
                this.localStream.getTracks().forEach(track => track.stop());
            } catch (e) {
                console.warn('Gagal menghentikan track localStream:', e);
            }
            this.localStream = null;
        }

        // 4. Matikan screen share stream tracks
        if (this.screenStream) {
            try {
                this.screenStream.getTracks().forEach(track => track.stop());
            } catch (e) {
                console.warn('Gagal menghentikan track screenStream:', e);
            }
            this.screenStream = null;
        }

        // 5. Tutup PeerConnection
        if (this.peerConnection) {
            try {
                this.peerConnection.close();
            } catch (e) {
                console.warn('Gagal menutup peerConnection:', e);
            }
            this.peerConnection = null;
        }

        // 6. Lepaskan session heartbeat
        if (this._sessionHeartbeat) {
            clearInterval(this._sessionHeartbeat);
            this._sessionHeartbeat = null;
        }

        // 7. Bersihkan event listeners
        ...
        // 8. Hapus session lock dari localStorage
        localStorage.removeItem(this.callSessionKey);

        // 9. Reset status internal ke idle
        this.status = 'idle';
        this.callId = null;
        this.role = null;

        // 10. Hapus UI overlay jika ada
    ...
    async pollCallStatus() {
        if (!navigator.onLine) {
            console.log('[Call] Offline, menunda polling status panggilan.');
            return;
        }
        try {
            ...
            if (this.status === 'idle') {
                ...
                // Self-healing: Jika server mendeteksi status idle, pastikan lock lokal dibersihkan
                if (data.status === 'idle' || !data.status) {
                    if (this._isSessionOwner()) {
                        console.log('[Call] Server menyatakan panggilan idle. Menghapus local session lock.');
                ...
                if (data.status === 'ringing' && data.role === 'caller') {
                    ...
                    if (data.status === 'active') {
                        if (this.role === 'caller') {
                            ...
                            if (data.sdp_answer && this.normalizeSdp(data.sdp_answer) !== this.normalizeSdp(this.currentSdpAnswer)) {
                            ...
                            if (data.sdp_offer && this.normalizeSdp(data.sdp_offer) !== this.normalizeSdp(this.currentSdpOffer)) {
                    ...
                    if (this.peerConnection) {
                        ...
                        if (ice === 'failed' || conn === 'failed' || ice === 'disconnected') {
                            ...
                            if (!this._disconnectStartTime) {
                                ...
                                console.warn(`[Call] Koneksi terputus (ICE: ${ice}, Conn: ${conn}). Memulai toleransi pemulihan 15 detik...`);
                            ...
                            // Jika terputus sudah lebih dari 15 detik ATAU status koneksi failed permanen
                            if (elapsed >= 15000 || ice === 'failed' || conn === 'failed') {
                                ...
                                // Jangan stop track kamera/mic lokal agar bisa langsung digunakan ulang secara instan tanpa delay inisialisasi hardware
                                if (this.peerConnection) {
                                    try {
                                        this.peerConnection.close();
                                    } catch (e) {}
    ...
    async reconnectActiveCall(data) {
        ...
        try {
            // Ambil media dengan constraints dinamis (Reuse stream jika sudah ada yang aktif)
            ...
            if (!hasActiveTracks) {
                ...
                console.log('[Call Reconnect] Menggunakan kembali localStream yang sudah aktif.');
                this.callType = data.call_type || data.type || 'audio';
            }
    ...
    async startCallFlow(type = 'audio') {
        ...
        try {
            ...
            // Coba getUserMedia — jika kamera gagal (ditolak/tidak ada/HTTP), fallback ke audio-only
            try {
                this.localStream = await navigator.mediaDevices.getUserMedia(constraints);
            } catch (mediaErr) {
                if (this.callType === 'video' && constraints.video) {
                    console.warn('[Call Caller] Kamera gagal diakses, fallback ke audio-only:', mediaErr.name, mediaErr.message);
                    // Downgrade ke audio-only, beri tahu user
                    this.callType = 'audio';
                    this.updateVideoUI();
                    try {
                        this.localStream = await navigator.mediaDevices.getUserMedia({ audio: this.getAudioConstraints() });
                    } catch (audioErr) {
                        // Bahkan audio pun gagal — ini sudah parah, rethrow agar outer catch tangani
                        throw audioErr;
                    }
                    if (window.showAlert) {
                        window.showAlert('Kamera tidak dapat diakses. Panggilan dilanjutkan dengan suara saja.', 'Kamera Tidak Tersedia', 'warning');
                    }
                } else {
                    throw mediaErr; // Audio juga gagal, biarkan outer catch menangani
                }
            }
            ...
            if (this.callType === 'video') {
                ...
                if (localVideo) {
                    localVideo.srcObject = this.localStream;
                    localVideo.play().catch(e => console.warn('[Call] localVideo.play() gagal (caller):', e));
                }
            }
    ...
    async acceptCallFlow() {
        ...
        try {
            ...
            // Coba getUserMedia — jika kamera gagal (ditolak/tidak ada/HTTP), fallback ke audio-only
            // PENTING: tanpa try-catch ini, kegagalan kamera akan menutup seluruh panggilan secara diam-diam
            try {
                this.localStream = await navigator.mediaDevices.getUserMedia(constraints);
            } catch (mediaErr) {
                if (this.callType === 'video' && constraints.video) {
                    console.warn('[Call Receiver] Kamera gagal diakses, fallback ke audio-only:', mediaErr.name, mediaErr.message);
                    // Downgrade ke audio-only agar panggilan tetap berjalan
                    this.callType = 'audio';
                    this.updateVideoUI();
                    try {
                        this.localStream = await navigator.mediaDevices.getUserMedia({ audio: this.getAudioConstraints() });
                    } catch (audioErr) {
                        // Bahkan audio pun gagal — rethrow agar outer catch menangani
                        throw audioErr;
                    }
                    if (window.showAlert) {
                        window.showAlert('Kamera tidak dapat diakses. Panggilan dilanjutkan dengan suara saja.', 'Kamera Tidak Tersedia', 'warning');
            ...
            if (this.callType === 'video') {
                ...
                if (localVideo) {
                    localVideo.srcObject = this.localStream;
                    localVideo.play().catch(e => console.warn('[Call] localVideo.play() gagal (receiver):', e));
                }
            }
    ...
    async toggleScreenShare() {
        if (typeof window.showAlert === 'function') {
            window.showAlert('Fitur berbagi layar sekarang berada di menu tersendiri "Layar" untuk performa native WebRTC maksimal. Anda akan diarahkan ke halaman tersebut.', 'Info', 'info');
        } else {
            alert('Fitur berbagi layar sekarang berada di menu tersendiri "Layar" untuk performa native WebRTC maksimal.');
        }
        setTimeout(() => {
            window.location.href = this.baseUrl.replace('/public', '') + '/layarbersama';
        }, 2000);
    }
        ...
        if (isApk) {
            if (window.Capacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.ScreenShare) {
                try {
                    ...
                    const captureOpts = useGamingMode ? { scale: 0.55, fps: 30, quality: 65 } : { scale: 0.45, fps: 25, quality: 50 };
                    ...
                    if (res && res.port) {
                        ...
                        const processNextFrame = async () => {
                            ...
                            if (isProcessingFrame || frameQueue.length === 0) return;
                            isProcessingFrame = true;

                            // Frame skipping: Jika antrean menumpuk, hanya simpan 1 frame terbaru untuk menjaga latensi tetap 0ms
                            ...
                            try {
                                ...
                                bitmap.close(); // Hancurkan bitmap instan agar RAM dibebaskan!

                                if (!isStreamInitialized || resolutionChanged) {
                                    const isFirstInit = !isStreamInitialized;
                                    isStreamInitialized = true;

                                    // Hentikan track lama agar tidak bertumpuk
                                    if (this.screenStream) {
                                    ...
                                    // Buat ulang stream dengan resolusi canvas yang baru (on-demand capture)
                                    this.screenStream = canvas.captureStream();
                                    const newScreenTrack = this.screenStream.getVideoTracks()[0];
                                    if (newScreenTrack && 'contentHint' in newScreenTrack) {
                                        newScreenTrack.contentHint = useGamingMode ? 'motion' : 'detail';
                                    }
                                    ...
                                    if (newScreenTrack) {
                                        ...
                                        if (this.peerConnection) {
                                            ...
                                            if (videoSender) {
                                                ...
                                                console.log('[Native ScreenShare] Video track peer connection berhasil diupdate.');
                                                try {
                                                    ...
                                                    if (parameters.encodings.length > 0) {
                                                        parameters.encodings[0].maxBitrate = useGamingMode ? 1000000 : 400000;
                                                        parameters.encodings[0].maxFramerate = useGamingMode ? 30 : 25;
                                                        await videoSender.setParameters(parameters);
                                                        console.log(`[WebRTC] Parameter encoding berhasil dioptimalkan.`);
                                                    }
                                                } catch (paramErr) {
                                ...
                                console.error('[Native ScreenShare] Gagal memproses frame WebSocket:', e);
                            } finally {
                        ...
                        ws.onopen = () => {
                            console.log('[Native ScreenShare] WebSocket terhubung.');
                        };
                        ...
                        ws.onerror = (err) => {
                            console.error('[Native ScreenShare] WebSocket error:', err);
                        };

                        ws.onclose = () => {
                            console.log('[Native ScreenShare] WebSocket tertutup.');
                        };
                        return;
                    }
                } catch (err) {
    ...
    _preferCodecInSdp(sdp, codecName) {
        if (!sdp) return sdp;
        // Deteksi pembatas baris secara dinamis (\r\n atau \n)
        const isLf = sdp.indexOf('\r\n') === -1 && sdp.indexOf('\n') !== -1;
    ...
    _optimizeSdpCodecs(sdp) {
        // Prioritaskan H264 (akselerasi hardware universal di mobile) dibanding VP8/VP9/AV1
        let optSdp = this._preferCodecInSdp(sdp, 'VP8');
        optSdp = this._preferCodecInSdp(optSdp, 'H264');
        return optSdp;
    }
    ...
    async stopScreenShare() {
        ...
        if (this._mjpegWs) {
            try { this._mjpegWs.close(); } catch (e) {}
            this._mjpegWs = null;
        }
        if (this._mjpegImg) {
            this._mjpegImg.src = "";
        ...
        if (this.localStream && this.peerConnection) {
            ...
            if (cameraTrack) {
                ...
                if (videoSender) {
                    ...
                    // Hapus batasan bitrate/framerate agar kualitas kamera kembali normal
                    try {
                        const parameters = videoSender.getParameters();
