...
class RelationshipCall {
        ...
        if (isApk) {
            if (window.Capacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.ScreenShare) {
                try {
                    ...
                    const captureOpts = useGamingMode ? { scale: 0.50, fps: 24, quality: 60 } : { scale: 0.35, fps: 12, quality: 45 };
                    ...
                    if (res && res.port) {
                        ...
                        let lastDrawTime = 0;
                        const frameInterval = useGamingMode ? 41 : 83;
                        ...
                        const drawFrame = (now) => {
                            ...
                            if (mjpegImg.naturalWidth) {
                                if (canvas.width !== mjpegImg.naturalWidth || canvas.height !== mjpegImg.naturalHeight) {
                                    ...
                                    // Hentikan track lama agar tidak bertumpuk (hapus listener onended agar tidak men-trigger stopScreenShare secara tidak sengaja)
                                    if (this.screenStream) {
                                    ...
                                    // Buat ulang stream dengan resolusi canvas yang baru
                                    this.screenStream = canvas.captureStream(useGamingMode ? 24 : 12);
                                        ...
                                        if (this.peerConnection) {
                                            ...
                                            if (videoSender) {
                                                videoSender.replaceTrack(newScreenTrack)
                                                    .then(() => {
                                                        console.log('[Native ScreenShare] Video track peer connection berhasil diupdate setelah rotasi.');
                                                        try {
                                                            ...
                                                            if (parameters.encodings.length > 0) {
                                                                parameters.encodings[0].maxBitrate = useGamingMode ? 1000000 : 300000;
                                                                parameters.encodings[0].maxFramerate = useGamingMode ? 24 : 12;
                                                                videoSender.setParameters(parameters);
                                                                console.log(`[WebRTC] Parameter encoding berhasil dioptimalkan setelah rotasi.`);
                                                            }
                                                        } catch (paramErr) {
                                                            console.warn('[WebRTC] Gagal menyetel parameter videoSender setelah rotasi:', paramErr);
                                                        }
                                                    })
                                                    .catch(err => {
                                                        console.error('[Native ScreenShare] Gagal replaceTrack setelah rotasi:', err);
                                                    });
                                            }
                                        }
                                    }
                                }
                                // Hapus ctx.filter karena filter CSS di canvas sangat membebani CPU/GPU WebView
                            ...
                            if (videoSender) {
                                ...
                                try {
                                    ...
                                    if (parameters.encodings.length > 0) {
                                        ...
                                        console.log(`[WebRTC] Parameter encoding berhasil dioptimalkan (Max: ${parameters.encodings[0].maxBitrate} bps, ${parameters.encodings[0].maxFramerate} FPS).`);
                                    }
                                } catch (paramErr) {
    ...
    _optimizeSdpCodecs(sdp) {
        // Prioritaskan H264 (akselerasi hardware universal di mobile) & VP8 dibanding AV1/VP9 (software-heavy)
        let optSdp = this._preferCodecInSdp(sdp, 'H264');
        optSdp = this._preferCodecInSdp(optSdp, 'VP8');
        return optSdp;
    }
