...
class RelationshipCall {
    ...
    handleRemoteScreenShareToggle(active) {
        ...
        if (active) {
            ...
            // Tampilkan kembali header & kontrol expand modal jika tadinya disembunyikan
            const expandHeader = document.getElementById('call-expand-header');
            const expandControls = document.getElementById('call-expand-controls');
            if (expandHeader) {
                expandHeader.style.removeProperty('display');
            }
            if (expandControls) {
                expandControls.style.removeProperty('display');
            }
    ...
    adjustScreenShareLayout() {
        ...
        // ── Deteksi Rotasi Fisik Layar Penerima (Viewer) ──
        // Jika layar penerima memanjang ke samping (landscape), dan stream dari pengirim berupa portrait (vertikal)
        // karena kendala WebRTC canvas stream, gunakan 'object-cover' agar melakukan zoom-in sehingga konten landscape 
        // di dalam stream portrait tersebut memenuhi lebar layar (tidak mengecil/double-letterbox).
        ...
        [remoteVideo, overlayVideo].forEach(v => {
            ...
            if (!isStreamLandscape && isScreenLandscape) {
                v.classList.remove('object-contain');
                v.classList.add('object-cover');
            } else {
        ...
        // ── Deteksi Rotasi Fisik Layar Penerima (Viewer) ──
        // Jika layar penerima memanjang ke samping (landscape), sembunyikan kontrol UI obrolan (True Fullscreen)
        // Jika layar penerima tegak (portrait), tampilkan kembali kontrol UI obrolan di atas & bawah video stream
        console.log(`[Discord Layout] Screen: ${isScreenLandscape ? 'Landscape' : 'Portrait'}, Stream: ${isStreamLandscape ? 'Landscape' : 'Portrait'}`);

        if (isScreenLandscape) {
            // Mode True Fullscreen: Sembunyikan kontrol agar video mengambil ruang maksimal
            if (expandHeader) {
                expandHeader.style.setProperty('display', 'none', 'important');
            }
            if (expandControls) {
                expandControls.style.setProperty('display', 'none', 'important');
            }
        } else {
