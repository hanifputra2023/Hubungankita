...
class RelationshipCall {
    ...
    handleRemoteScreenShareToggle(active) {
        ...
        if (active) {
            // ── Langkah 1: switch ke object-contain dulu (tidak terpotong) ──
            ...
            // ── Langkah 2: simpan class asal untuk restore nanti ──
            ...
            // ── Langkah 3: Matikan padding card agar video punya ruang penuh ──
            if (expandCard) expandCard.style.padding = '0';

            // ── Langkah 4: Deteksi orientasi otomatis dari dimensi stream ──
            // Panggil segera + pasang event resize agar bisa adaptasi saat game
            // beralih dari portrait ke landscape di tengah sesi
            this._adaptRemoteVideoLayout();

            if (remoteVideo && !remoteVideo._screenShareResizeHandler) {
                remoteVideo._screenShareResizeHandler = () => {
                    if (this._remoteScreenSharing) this._adaptRemoteVideoLayout();
                };
                remoteVideo.addEventListener('resize', remoteVideo._screenShareResizeHandler);
            }

        } else {
            // ── Cabut event listener resize ──
            if (remoteVideo && remoteVideo._screenShareResizeHandler) {
                remoteVideo.removeEventListener('resize', remoteVideo._screenShareResizeHandler);
                remoteVideo._screenShareResizeHandler = null;
            }

            // ── Kembalikan video ke object-cover (video call biasa) ──
            [remoteVideo, overlayVideo].forEach(v => {
                if (!v) return;
    ...
    /**
     * Baca dimensi aktual video remote → sesuaikan card & overlay.
     * Landscape (game): card melebar. Portrait (HP normal): card memanjang.
     * Retry otomatis jika video belum siap streaming.
     */
    _adaptRemoteVideoLayout() {
        ...
        if (!vw || !vh) {
            // Stream belum ada dimensi — tunggu sebentar lalu coba lagi
            setTimeout(() => {
                if (this._remoteScreenSharing) this._adaptRemoteVideoLayout();
            }, 400);
            return;
        ...
        if (expandCard) {
            if (isLandscape) {
                // ── LANDSCAPE (game, video horizontal) ──
                // Card: lebar, tidak terlalu tinggi — mirip layar game
                expandCard.classList.remove('max-w-sm', 'max-w-3xl');
                expandCard.classList.add('max-w-3xl');
                expandCard.style.minHeight   = '';
                expandCard.style.height      = '';
                expandCard.style.maxHeight   = '90dvh';
                expandCard.style.width       = 'min(90vw, 960px)';
                expandCard.style.aspectRatio = '';
            } else {
                // ── PORTRAIT (HP normal, 9:16) ──
                // Card: narrow tapi tinggi — ikut bentuk layar HP
                expandCard.classList.remove('max-w-sm', 'max-w-3xl');
        ...
        if (overlayContainer) {
            ...
            if (isLandscape) {
                ...
                overlayContainer.style.height      = '260px';
            }
        }
    }
