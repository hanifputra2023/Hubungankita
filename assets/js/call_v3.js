...
class RelationshipCall {
    ...
    adjustScreenShareLayout() {
        ...
        if (remoteVideo.videoWidth === 0 || remoteVideo.videoHeight === 0) {
            ...
            // Default containment sebelum loading dimensi selesai
            [remoteVideo, overlayVideo].forEach(v => {
                if (v) {
            ...
            const isLandscapeStream = remoteVideo.videoWidth > remoteVideo.videoHeight;
            const isLandscapeScreen = window.innerWidth > window.innerHeight;
            const orientationsMatch = isLandscapeStream === isLandscapeScreen;

            console.log(`[ScreenShare Layout] Stream: ${isLandscapeStream ? 'Landscape' : 'Portrait'}, Screen: ${isLandscapeScreen ? 'Landscape' : 'Portrait'}, Match: ${orientationsMatch}`);

            [remoteVideo, overlayVideo].forEach(v => {
                if (!v) return;
                if (orientationsMatch) {
                    // Orientasi cocok (Landscape on Landscape / Portrait on Portrait): 
                    // gunakan object-cover agar gambar penuh (zoom/fill) menutup seluruh celah hitam
                    v.classList.remove('object-contain');
                    v.classList.add('object-cover');
                } else {
                    // Orientasi berbeda (Landscape on Portrait):
                    // gunakan object-contain agar konten lebar tidak terpotong ekstrem di layar tinggi
                    v.classList.remove('object-cover');
                    v.classList.add('object-contain');
                }
            });
