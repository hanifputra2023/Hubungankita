// ============================================================
// public/assets/js/call.js
// Core WebRTC Voice Calling & Signaling System (HTTP Polling)
// ============================================================
...
class RelationshipCall {
    ...
    bindEvents() {
        ...
        // ── Single Tap to Toggle Controls in Video Call ──
        const expandCard = document.getElementById('call-expand-card');
        if (expandCard) {
            expandCard.addEventListener('click', (e) => {
                // Only act if the current call type is video and call is active or ringing
                if (this.callType !== 'video' || this.status === 'idle') return;
        ...
        try {
            ...
            if (data.status === 'ringing' && data.role === 'receiver') {
                ...
                if (this.status !== 'ringing' || this.callId !== data.call_id) {
                    // Reset state stale sebelum tampilkan incoming
                    if (this.status !== 'idle') {
                        this.terminateCallLocally();
                    }
    ...
    showIncomingUI(data) {
        ...
        if (callerAvatar) {
            ...
            if (avatarUrl && avatarUrl.startsWith('http://') && window.location.protocol === 'https:') {
    ...
    waitForIceGathering() {
        ...
        return new Promise((resolve) => {
            ...
            const timeout = setTimeout(done, 200); // Maksimal tunggu hanya 200ms (Hybrid ICE) demi kecepatan instan!
        });
    }
    ...
    updateVideoUI() {
        const isVideo = this.callType === 'video' && this.status !== 'idle';
    ...
    toggleVideoControls() {
        ...
        const isHidden = header.classList.contains('opacity-0');
        if (isHidden) {
    ...
    async startScreenShare() {
        ...
        if (isApk) {
            if (window.Capacitor && window.Capacitor.Plugins && window.Capacitor.Plugins.ScreenShare) {
                try {
                    console.log('[Native ScreenShare] Menginisialisasi screen capture...');
                    const res = await window.Capacitor.Plugins.ScreenShare.startCapture();
                    if (res && res.port) {
                        const port = res.port;
                        console.log('[Native ScreenShare] Server MJPEG aktif di port:', port);

                        const mjpegImg = new Image();
                        mjpegImg.crossOrigin = "anonymous";
                        
                        // Sembunyikan elemen gambar secara off-screen agar browser tetap merender frame
                        mjpegImg.style.position = 'absolute';
                        mjpegImg.style.left = '-9999px';
                        mjpegImg.style.top = '-9999px';
                        mjpegImg.style.width = '1px';
                        mjpegImg.style.height = '1px';
                        mjpegImg.style.opacity = '0.01';
                        ...
                        canvas.width = 360;
                        canvas.height = 640;
                        // Sembunyikan canvas secara off-screen
                        ...
                        let active = true;
                        this._mjpegImg = mjpegImg;
                        this._mjpegCanvas = canvas;
                        this._mjpegActive = () => { active = false; };

                        const drawFrame = () => {
                            if (!active) return;
                            if (mjpegImg.naturalWidth) {
                                if (canvas.width !== mjpegImg.naturalWidth || canvas.height !== mjpegImg.naturalHeight) {
                                    canvas.width = mjpegImg.naturalWidth;
                                    canvas.height = mjpegImg.naturalHeight;
                                    console.log('[Native ScreenShare] Resolusi canvas disesuaikan ke:', canvas.width, 'x', canvas.height);
                                }
                                ctx.drawImage(mjpegImg, 0, 0);
                            }
                            // Gunakan setTimeout agar terus berjalan di background
                            setTimeout(drawFrame, 30);
                        };

                        mjpegImg.src = `http://127.0.0.1:${port}/`;
                        
                        // Tunggu loading frame pertama
                        await new Promise(resolve => setTimeout(resolve, 500));
                        drawFrame();

                        this.screenStream = canvas.captureStream(30);
                        ...
                        if (this.peerConnection) {
                            ...
                            if (videoSender) {
                                ...
                                console.log('[Native ScreenShare] Video track peer connection berhasil diganti.');
                            }
                        }
                        ...
                        screenTrack.onended = () => {
                            console.log('[Native ScreenShare] Track ended, stopping screen share.');
                            this.stopScreenShare();
                        };
                        return;
                    }
                } catch (err) {
                    console.error('[Native ScreenShare] Gagal memulai capture:', err);
                    if (window.showAlert) {
    ...
    async stopScreenShare() {
        ...
        if (this._mjpegActive) {
            this._mjpegActive();
            this._mjpegActive = null;
        }
        if (this._mjpegImg) {
            this._mjpegImg.src = "";
            if (this._mjpegImg.parentNode) {
                this._mjpegImg.parentNode.removeChild(this._mjpegImg);
            }
            this._mjpegImg = null;
        }
        if (this._mjpegCanvas) {
            if (this._mjpegCanvas.parentNode) {
                this._mjpegCanvas.parentNode.removeChild(this._mjpegCanvas);
            }
            this._mjpegCanvas = null;
        }
    ...
    handleRemoteScreenShareToggle(active) {
        ...
        this._remoteScreenSharing = active;
        ...
        if (active) {
            // Tampilkan container video & sembunyikan avatar (jika dari panggilan audio)
            if (videoContainer) videoContainer.classList.remove('hidden');
            if (avatarContainer) avatarContainer.classList.add('hidden');

            // ── 1. Video: object-contain agar tidak ada yang terpotong ──
            ...
            // ── 2. Simpan class/style asal ──
            if (expandModal  && !expandModal.dataset.ssOrig)  expandModal.dataset.ssOrig  = expandModal.getAttribute('style') || '';
            if (expandCard   && !expandCard.dataset.ssOrig)   { expandCard.dataset.ssOrig = expandCard.className; }
            if (header       && !header.dataset.ssOrig)       header.dataset.ssOrig       = header.getAttribute('style') || '';
            if (controls     && !controls.dataset.ssOrig)     controls.dataset.ssOrig     = controls.getAttribute('style') || '';
            if (overlayContainer && !overlayContainer.dataset.ssOrig) overlayContainer.dataset.ssOrig = overlayContainer.className;

            // ── 3. Modal: hilangkan padding desktop ──
            if (expandModal) expandModal.style.padding = '0';

            // ── 4. Card: FULL SCREEN — hapus semua constraint ──
            if (expandCard) {
                // Hapus semua kelas lebar/padding bawaan Tailwind dari card
                expandCard.classList.remove(
                    'max-w-sm', 'max-w-md', 'max-w-lg', 'max-w-xl', 'max-w-2xl', 'max-w-3xl',
                    'rounded-3xl', 'p-6', 'md:p-8', 'p-8'
                );
                // Buat card mengisi seluruh modal
                Object.assign(expandCard.style, {
                    width:        '100%',
                    height:       '100%',
                    maxWidth:     'none',
                    maxHeight:    'none',
            ...
            // ── 5. Header & Controls: floating overlay di atas video ──
            if (header) {
            ...
            // ── 6. Overlay widget: tinggi dinamis, aspek dari stream ──
            if (overlayContainer) {
                overlayContainer.classList.remove('aspect-video');
                overlayContainer.style.height = '180px';
                overlayContainer.style.maxHeight = '180px';
            }

            // ── 7. Jalankan deteksi orientasi awal & polling interval ──
            this._lastSSLayoutState = null;
            this._adaptRemoteVideoLayout();

            if (!this._ssPollInterval) {
                this._ssPollInterval = setInterval(() => {
                    if (this._remoteScreenSharing) {
                        this._adaptRemoteVideoLayout();
                    } else {
                        clearInterval(this._ssPollInterval);
                        this._ssPollInterval = null;
                    }
                }, 500);
            }

            if (!this._ssWindowResizeHandler) {
                this._ssWindowResizeHandler = () => {
                    if (this._remoteScreenSharing) this._adaptRemoteVideoLayout();
                };
                window.addEventListener('resize', this._ssWindowResizeHandler);
                window.addEventListener('orientationchange', this._ssWindowResizeHandler);
            }

        } else {
            // ── Hentikan polling interval ──
            if (this._ssPollInterval) {
                clearInterval(this._ssPollInterval);
                this._ssPollInterval = null;
            }
            ...
            if (this._ssWindowResizeHandler) {
                window.removeEventListener('resize', this._ssWindowResizeHandler);
                window.removeEventListener('orientationchange', this._ssWindowResizeHandler);
                this._ssWindowResizeHandler = null;
            }

            this._lastSSLayoutState = null;

            // Kembalikan visibilitas berdasarkan tipe panggilan asli (audio/video)
            ...
            if (expandModal) {
                expandModal.style.padding = expandModal.dataset.ssOrig || '';
                delete expandModal.dataset.ssOrig;
            }

            // ── Restore expand card ke class asal ──
            if (expandCard && expandCard.dataset.ssOrig) {
                expandCard.className = expandCard.dataset.ssOrig;
                delete expandCard.dataset.ssOrig;
                ['width','height','maxWidth','maxHeight','minHeight','borderRadius','padding','margin','background']
                    .forEach(p => expandCard.style[p] = '');
            }

            // ── Restore header & controls ──
            [header, controls].forEach(el => {
                if (!el) return;
                ['position','top','left','right','bottom','zIndex','padding','background','borderTop','marginTop']
                    .forEach(p => el.style[p] = '');
                if (el.dataset.ssOrig !== undefined) delete el.dataset.ssOrig;
            });

            // ── Restore overlay container ──
            if (overlayContainer && overlayContainer.dataset.ssOrig) {
                overlayContainer.className = overlayContainer.dataset.ssOrig;
                delete overlayContainer.dataset.ssOrig;
    ...
    /**
     * Baca dimensi aktual video stream yang diterima.
     * Sesuaikan layout video (rotasi 90 derajat jika landscape di viewport portrait)
     * secara realtime dengan optimasi state caching.
     */
    _adaptRemoteVideoLayout() {
        const remoteVideo      = document.getElementById('remote-video');
        const overlayContainer = document.getElementById('call-overlay-video-container');

        const vw = remoteVideo ? remoteVideo.videoWidth  : 0;
        const vh = remoteVideo ? remoteVideo.videoHeight : 0;

        if (!vw || !vh) {
            return; // Biarkan interval memanggil kembali ketika metadata video sudah siap
        }

        const isLandscapeStream = vw > vh;
        const viewportWidth = window.innerWidth;
        const viewportHeight = window.innerHeight;
        const isViewportPortrait = viewportWidth < viewportHeight;

        // Optimasi: Hanya update DOM jika ada perubahan resolusi stream atau orientasi viewport
        const stateKey = `${vw}x${vh}_vp_${viewportWidth}x${viewportHeight}`;
        if (this._lastSSLayoutState === stateKey) {
            return;
        }
        this._lastSSLayoutState = stateKey;

        console.log(`[ScreenShare] Adaptasi layout: Stream ${vw}x${vh}, Viewport ${viewportWidth}x${viewportHeight}`);

        if (remoteVideo) {
            if (isLandscapeStream && isViewportPortrait) {
                // Sisi penerima di portrait, stream dari pengirim landscape -> ROTASI 90 derajat agar FULL SCREEN
                Object.assign(remoteVideo.style, {
                    position: 'absolute',
                    left: '50%',
                    top: '50%',
                ...
                console.log('[ScreenShare] Menerapkan rotasi video 90 derajat.');
            } else {
                // Reset style jika viewport sudah landscape atau stream portrait
                Object.assign(remoteVideo.style, {
                    position: '',
                    left: '',
                    top: '',
                ...
                console.log('[ScreenShare] Mereset style video (tampilan standar).');
            }
        }

        // Sesuaikan overlay widget aspect ratio dari stream nyata
        if (overlayContainer) {
            overlayContainer.classList.remove('aspect-video');
            overlayContainer.style.aspectRatio = `${vw} / ${vh}`;
            ...
            if (isLandscape) {
                ...
                overlayContainer.style.height    = '240px';
                overlayContainer.style.maxHeight = '240px';
            }
        }
    }
    ...
    openExpandModal() {
        ...
        // Jika partner sedang screen share, terapkan ulang layout portrait agar modal
        // tetap pas meski user baru buka expand modal di tengah sesi.
        if (this._remoteScreenSharing) {
            this.handleRemoteScreenShareToggle(true);
        }
    }
    ...
    updateOverlayDetails(partnerName, partnerAvatar) {
        ...
        if (partnerAvatar && partnerAvatar.startsWith('http://') && window.location.protocol === 'https:') {
            partnerAvatar = partnerAvatar.replace('http://', 'https://');
        }
